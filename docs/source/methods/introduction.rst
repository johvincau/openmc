...
OpenMC uses batch statistics. That is, OpenMC counts batches, which are 
the aggregated results (tallies) of generations of simulated particles, as 
individual realizations. See :ref:`Statistics <tallies_statistics>` for 
further background or :ref:`Execution Settings <usersguide_particles>` for 
configuring statistically sound simulations.
